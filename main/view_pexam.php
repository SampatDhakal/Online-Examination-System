...
if(isset($_GET["ec"]))
{
    $ec = $_GET["ec"];
    $object->query = "
    SELECT exam_id, exam_result_datetime FROM pexam_oea 
    ...
    foreach($result as $row)
    {
        ...
        WHERE subject_wise_exam_projectdetail.exam_id = '".$row["exam_id"]."'
        ";
                            ...
                            ?>
                                <table class="table table-bordered">
                                    ...
                                    foreach($exam_result as $row)
                                    {
                                        ...
                                        $duration = $object->Get_pexam_duration($row["exam_id"]) . ' minute';
                                        ...
                                        if(time() >= strtotime($start_time) && time() <= strtotime($end_time))
                                        {
                                            ...
                                            $action_button = '<button type="button" class="btn btn-primary btn-sm view_subject_pexam" data-ec="'.$ec.'" data-esc="'.$row["subject_exam_code"].'"><i class="fas fa-pencil-alt"></i></button>';
                                        }
                                        else
                                        {
                                            if(time() > strtotime($end_time))
                                            {
                                                ...
                                                else
                                                {
                                                    ...
                                                    {
                                                        $action_button = '<a href="single_presult.php?ec='.$ec.'&esc='.$row["subject_exam_code"].'" target="_blank" class="btn btn-danger btn-sm">Result</a>';
                                                    }
                                                    else
                                                    {
                                                        $action_button = '';
...
$(document).ready(function(){
    ...
    $('.view_subject_pexam').click(function(){
        var ec = $(this).data('ec');
        var esc = $(this).data('esc');
        $.ajax({
            url:"ajax_action.php",
            method:"POST",
            data:{page:"view_pexam", action:"view_subject_pexam", ec:ec, esc:esc},
