...
if(isset($_POST['page']))
{
			...
			else
			{
				...
				foreach($result as $row)
				{
					if($row['student_email_verified'] == 'Yes')
					{
						...
						{
							$error = '<div class="alert alert-danger">Sorry, Your account has been disabled, contact Admin</div>';
						}
					}
					else
					{
						$error = '<div class="alert alert-danger">You have not verify your email address, so for email verification, click <a href="resend_email_verification.php">here</a></div>';
					}
				}
			}
			...
			else
			{
				...
				foreach($result as $row)
				{
					if($row['student_email_verified'] == 'No')
					{
						...
						$subject = 'Student Verification email for Web Based Online Examination Application';

						$body = '
						<p>Hello '.$row["student_name"].'.</p>
						<p>This is a verification email for your '.$row["student_email_id"].' email address, and without email verification you can not take part in online exam, So please verify your email address by clicking this <a href="'.$object->base_url.'send_password.php?type=student&code='.$verification_code.'" target="_blank"><b>link</b></a>.</p>
						<p>In case if you have any difficulty please email us.</p>
						<p>Thank you,</p>
						<p>Web Based Online Examination Application</p>
						';
					...
					{
						$error = '<div class="alert alert-info">Your email is already verified, so you can login into system, by clicking <a href="login.php">here</a></div>';
					}
				}
			}
				...
				foreach($result as $row)
				{
					if($row['student_email_verified'] == 'Yes')
					{
						if($row["student_status"] == 'Enable')
						{
							...
							$subject = 'Web Based Online Examination Application Password Detail';

							$body = '
							<p>Hello '.$row["student_name"].'.</p>
							<p>For login into this Web Based Online Examination System by visiting <a href="'.$object->base_url.'login.php" target="_blank"><b>'.$object->base_url.'login.php</a></b> this link. Below you can find password details.</a></p>
	...
	if($_POST['page'] == 'forgetpassword_admin')
	{
		if($_POST['action'] == 'get_adminpassword')
		{
			sleep(2);
			$error = '';
			$data = array(
			...
			else
			{
				...
				foreach($result as $row)
				{
						...
						if($row["user_status"] == 'Enable')
						{
							...
							<p>For login into this Web Based Online Examination System by visiting <a href="'.$object->base_url.'admin/index.php" target="_blank"><b>'.$object->base_url.'admin/index.php</a></b> this link. Below you can find password details.</a></p>
	...
	if($_POST['page'] == 'exam')
	{
		...
		if($_POST["action"] == 'fetch_timetable')
		{
			...
			ON subject_oea.subject_id = subject_wise_exam_detail.subject_id 
...
if($_POST['page'] == 'pexam')
	{
		if($_POST["action"] == 'fetch')
		{
			...
			foreach($result as $row)
			{
					...
					else
					{
						if(time() > strtotime($exam_last_subject_end_datetime))
						{
							...
							if($exam_result_datetime != 'Not Publish')
							{
								if(time() > strtotime($exam_result_datetime))
								{
									$action_button = '<a href="pexam_result.php?ec='.$row["exam_code"].'" target="_blank" class="btn btn-danger btn-sm">Result</a>';
								}
								else
								{
									$action_button = '';
				...
				$sub_array[] = '<a href="view_pexam.php?ec='.$row["exam_code"].'">View</a>';
		...
		{
			$object->query = "
			SELECT * FROM subject_wise_exam_projcetdetail 
			INNER JOIN subject_oea 
			ON subject_oea.subject_id = subject_wise_exam_projectdetail.subject_id 
			WHERE subject_wise_exam_projcetdetail.exam_id = '".$_POST["exam_id"]."'
			";
			...
			foreach($result as $row)
			{
				$output .= '
					<tr>
						<td>'.$row["subject_name"].'</td>
						<td>'.$row["subject_exam_datetime"].'</td>
						<td>'.$object->Get_pexam_duration($_POST["exam_id"]).' Minute</td>
						<td>'.$row["subject_full_marks"].' Marks</td>
					</tr>
				';
			}
	...
	if($_POST['page'] == 'view_pexam')
	{
		if($_POST['action'] == 'view_subject_pexam')
		{
			$_SESSION['ec'] = $_POST['ec'];
			$_SESSION['esc'] = $_POST['esc'];
			echo 'view_subject_pexam.php';
		}
	}
	...
	if($_POST['page'] == 'view_subject_exam')
	{
		if($_POST['action'] == 'load_question')
		{
			...
			foreach($result as $row)
			{
				...
				$object->query = "
				SELECT * FROM question_option_oea 
				...
				';
				$object->query = "
				SELECT exam_subject_question_id FROM exam_subject_question_oea 
				WHERE exam_subject_question_id < '".$row['exam_subject_question_id']."' 
