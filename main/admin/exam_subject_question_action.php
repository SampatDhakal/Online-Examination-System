...
if(isset($_POST["action"]))
{
	...
	{
		$order_column = array('exam_oea.exam_title', 'subject_oea.subject_name', 'exam_subject_question_oea.exam_subject_question_title');
		...
		ON subject_wise_exam_detail.exam_subject_id = exam_subject_question_oea.exam_subject_id 
		INNER JOIN exam_oea 
		ON exam_oea.exam_id = subject_wise_exam_detail.exam_id 
		...
		{
			$search_query .= 'WHERE exam_oea.exam_title LIKE "%'.$_POST["search"]["value"].'%" ';
			$search_query .= 'OR subject_oea.subject_name LIKE "%'.$_POST["search"]["value"].'%" ';
			$search_query .= 'OR exam_subject_question_oea.exam_subject_question_title LIKE "%'.$_POST["search"]["value"].'%" ';
		}
		...
		{
			$order_query = 'ORDER BY exam_subject_question_oea.exam_subject_question_id DESC ';
		}
	...
	{
		$object->query = "
		SELECT subject_wise_exam_detail.exam_subject_id, subject_oea.subject_name 
		FROM subject_wise_exam_detail 
		INNER JOIN exam_oea 
		ON exam_oea.exam_id = subject_wise_exam_detail.exam_id 
		INNER JOIN subject_oea 
		ON subject_oea.subject_id = subject_wise_exam_detail.subject_id 
		WHERE exam_oea.exam_id = '".$_POST["exam_id"]."' 
	...
	if($_POST["action"] == 'Add')
	{
		...
		$object->query = "
		INSERT INTO exam_subject_question_oea 
		...
		for($count = 1; $count <= 4; $count++)
		{
			...
			$object->query = "
			INSERT INTO question_option_oea 
	...
	if($_POST["action"] == 'fetch_single')
	{
		...
		$object->query = "
		SELECT subject_oea.subject_name FROM subject_wise_exam_detail 
		INNER JOIN subject_oea 
		ON subject_oea.subject_id = subject_wise_exam_detail.subject_id 
	...
	if($_POST["action"] == 'Edit')
	{
		...
		$object->query = "
		UPDATE exam_subject_question_oea 
	...
	{
		$object->query = "
		DELETE FROM exam_subject_question_oea 
		...
		$object->query = "
		DELETE FROM question_option_oea 
		WHERE exam_subject_question_id = '".$_POST["id"]."'
		";
