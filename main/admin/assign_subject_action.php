...
if(isset($_POST["action"]))
{
	...
	{
		$order_column = array('class_oea.class_name', 'subject_oea.subject_name', 'subject_to_class_oea.added_on');

		$output = array();

		$main_query = "
		SELECT * FROM subject_to_class_oea 
		INNER JOIN class_oea 
		ON class_oea.class_id = subject_to_class_oea.class_id 
		...
		{
			$search_query .= 'WHERE subject_oea.subject_name LIKE "%'.$_POST["search"]["value"].'%" ';
			$search_query .= 'OR class_oea.class_name LIKE "%'.$_POST["search"]["value"].'%" ';
		}
		...
		{
			$order_query = 'ORDER BY subject_to_class_oea.subject_to_class_id DESC ';
		}
	...
	if($_POST["action"] == 'Add')
	{
		...
		else
		{
			...
			$object->query = "
			INSERT INTO subject_to_class_oea 
	...
	{
		$object->query = "
		DELETE FROM subject_to_class_oea 
