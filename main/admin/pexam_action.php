...
if(isset($_POST["action"]))
{
	...
	{
		$order_column = array('pexam_oea.exam_title', 'class_oea.class_name', 'pexam_oea.exam_duration', 'pexam_oea.exam_result_datetime', 'pexam_oea.exam_status');

		$output = array();

		$main_query = "
		SELECT * FROM pexam_oea 
		INNER JOIN class_oea 
		ON class_oea.class_id = pexam_oea.exam_class_id 
		";
		...
		{
			$search_query .= 'WHERE class_oea.class_name LIKE "%'.$_POST["search"]["value"].'%" ';
			$search_query .= 'OR pexam_oea.exam_title LIKE "%'.$_POST["search"]["value"].'%" ';
			$search_query .= 'OR pexam_oea.exam_duration LIKE "%'.$_POST["search"]["value"].'%" ';
			$search_query .= 'OR pexam_oea.exam_result_datetime LIKE "%'.$_POST["search"]["value"].'%" ';
			$search_query .= 'OR pexam_oea.exam_status LIKE "%'.$_POST["search"]["value"].'%" ';
		}
		...
		{
			$order_query = 'ORDER BY pexam_oea.exam_id DESC ';
		}
			...
			if($total_subject > 0)
			{
                ...
                else
                {
                	if(time() > $exam_last_subject_end_datetime)
                	{
	                    ...
	                    $action_button = '<button type="button" name="publish_result" class="btn btn-primary btn-sm publish_result" data-exam_id="'.$row["exam_id"].'">Publish Result</button>&nbsp;&nbsp;<a href="pexam_result.php?ec='.$row["exam_code"].'" class="btn btn-secondary btn-sm">View Result</a>';
	                }
	                else
	                {
